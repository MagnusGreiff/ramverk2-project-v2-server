...
ifneq ($(wildcard .nycrc),)
	[ ! -d test ] || $(NYC) $(MOCHA) --reporter dot 'test/**/*.js' --exit
else
	[! -d test ] || $(MOCHA) --reporter dot 'test/**/*.js' --exit
endif


# target: node8.6.0-test
.PHONY: test1
test1:
	docker-compose build node860-test && docker-compose run node860-test

# target: node8.3.0-test
.PHONY: test2
test2:
	docker-compose build node830-test && docker-compose run node830-test


# target: node8.1.0-test
.PHONY: test3
test3:
	docker-compose build node810-test && docker-compose run node810-test

# target: node8.6.0-test
.PHONY: start-docker
start-docker:
	docker-compose up -d --no-deps --build node860
